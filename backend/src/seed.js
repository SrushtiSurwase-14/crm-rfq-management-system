...
async function seed() {
  await sequelize.sync({ force: true }); // clean slate for demo purposes
  ...
  await Inventory.bulkCreate([
    ...
    { product_id: products[2].id, quantity_available: 50, reorder_level: 80, warehouse_location: 'WH-B' },  // deliberately low stock for demo
    ...
    { product_id: products[5].id, quantity_available: 0, reorder_level: 100, warehouse_location: 'WH-C' }, // out of stock for demo
  ]);
  ...
  // Demo RFQ 1: Sharma Pharma Multi-product RFQ with Draft Quotation
  ...
  const quote1 = await Quote.create({
    ...
    customer_id: customers[0].id,
    status: 'pending_approval',
    total_amount: 303500, // (500 * 185) + (2000 * 92) + (50 * 540) = 92500 + 184000 + 27000 = 303500
  });
  ...
  // Demo RFQ 2: Anita Verma CAS-specific RFQ ready for pricing
  ...
  console.log('\nSeed complete. Demo login credentials (password for all: Password@123):');
  console.log('  Admin    -> admin@demo.com');
  console.log('  Sales    -> sales@demo.com');
  console.log('  Reviewer -> reviewer@demo.com\n');
  console.log('Demo RFQs & Quote loaded:');
  console.log('  - RFQ #1: Sharma Pharma (3 line items, Quote #1 pending approval)');
  console.log('  - RFQ #2: Verma Chemicals (Ascorbic Acid CAS 50-81-7 extracted)\n');
